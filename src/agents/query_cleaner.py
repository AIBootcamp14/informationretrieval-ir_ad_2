...
def query_cleaner(state: Dict[str, Any], llm=None) -> Dict[str, str]:
    ...
    # 프롬프트 가져오기
    llm_manager = get_llm_manager()
    prompt = llm_manager.get_prompt('query_cleaner')
...
if __name__ == "__main__" :
    question1 = {'question' : "그럼 그거 물 속에서는 어덯게 돼?", 'messages' : [HumanMessage(content = "빛은 왜 직진해?"),
                                                                   AIMessage(content = "광자는 매질이 균일할 때 직진성을 보입니다.")]}
    question2 = {'question' : "아 그거는 왜 그렇게 된 거야?", 'messages' : [HumanMessage(content="지구 대길는 어떻게 구성돼 있어?"),
                                                                AIMessage(content = "질소 78%, 산소 21% 등이 포함됩니다.")]}
    
    output1 = query_cleaner(question1)
    output2 = query_cleaner(question2)
    
    print(f"rewritten question : {output1['rewritten_query']}")
    print(f"rewritten question : {output2['rewritten_query']}")
